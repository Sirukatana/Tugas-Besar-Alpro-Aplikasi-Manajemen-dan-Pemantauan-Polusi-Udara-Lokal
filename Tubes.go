...
// Mengautofill angka 0 di depan jika kurang dari 2 digit,
// memvalidasi bulan (01-12), hari sesuai bulan & tahun (pakai time.Date),
// dan meminta ulang jika format atau nilai tidak valid.
func inputTanggal() string {
	for {
		...
		// Validasi tahun (minimal 1000 agar 4 digit)
		...
		// Validasi hari: gunakan time.Date untuk deteksi otomatis
		// (termasuk tahun kabisat untuk Februari)
		// time.Date dengan hari=0 → hari terakhir bulan sebelumnya,
		// sehingga kita cek apakah tanggal yang di-parse tetap sama.
		tgl := time.Date(tahun, time.Month(bulan), hari, 0, 0, 0, 0, time.UTC)
		if tgl.Year() != tahun || int(tgl.Month()) != bulan || tgl.Day() != hari {
			// time.Date menerima overflow (misal hari=31 di bulan April
			// → rollover ke 1 Mei), kita deteksi ini dengan membandingkan
			// hasil parse dengan input asli.
		...
		// Autofill: format kembali dengan zero-padding
...
func menuPengurutan(data *tabpol, nData int) {
	...
	for {
		fmt.Println("\n--- Menu Pengurutan ---")
		fmt.Println("1. Selection Sort (berdasarkan AQI)")
		fmt.Println("2. Insertion Sort (berdasarkan Tanggal)")
		fmt.Println("0. Kembali")
		fmt.Print("Pilihan: ")
		pilihan = scanInt()
...
func sequentialSearch(data tabpol, nData int) {
	...
	for i := 0; i < nData; i++ {
		if data[i].kota == cari {
			fmt.Printf(">> Ditemukan: %s, %s | AQI: %.2f | Status: %s\n",
...
func binarySearch(data tabpol, nData int) {
	fmt.Print("Masukkan nama kota/kabupaten (data harus terurut): ")
	cari := titleCase(inputTeks())
	...
	for kiri <= kanan {
		...
		if data[tengah].kota == cari {
			fmt.Printf(">> Ditemukan: %s, %s | AQI: %.2f | Status: %s\n",
				data[tengah].kota, data[tengah].provinsi, data[tengah].aqi, kategoriPolusi(data[tengah].aqi))
			return
...
func kelompokPerProvinsi(data tabpol, nData int) {
	...
	// Kumpulkan daftar provinsi unik
